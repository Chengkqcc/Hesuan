<template>
	<view>
			...
			<view class="form-item" v-if="apply">
				...
				<view class="form-item-content radio">
					<uni-section title="选择报备申请人" type="line">
						<uni-data-select v-model="URDomesticReport.userReport.userId" :localdata="familyList"
		...
		<view class="address-info">
			<view class="form-item">
				...
				<view class="form-item-content" style="padding-top: 5rpx; box-sizing: border-box">
					<uni-data-picker :localdata="deptTree" placeholder="请选择" popup-title="请选择社区/网格" :readonly="isTrue"
						v-model="URDomesticReport.companyId" :clear-icon='false' :map="{text:'label',value:'id'}"
			...
			<view class="form-item">
				...
				<view class="form-item-content">
					<picker mode="region" @change="chooseLocation" :readonly="isTrue">
						<input v-model="URDomesticReport.originAddr" class="input" disabled placeholder="请选择" />
					</picker>
				</view>
			</view>
			<!-- <view class="form-item">
				...
				<view class="form-item-content textarea">
					<textarea v-model="URDomesticReport.originDetail" placeholder="请输入"></textarea>
				</view>
			...
			<view class="form-item">
				...
				<view class="form-item-content">
					<input @click="showDusinessType = true" v-model="URDomesticReport.transportationStr" disabled
			...
			<view class="form-item">
				...
				<view class="form-item-content">
					<input v-model="URDomesticReport.flight" :maxlength='50' class="input" placeholder="请输入"
						:readonly="isTrue" />
				</view>
			</view>
			<view class="form-item">
				...
				<view class="form-item-content textarea">
					<textarea v-model="URDomesticReport.destAddr" disabled placeholder="请不要使用默认加载地址，搜索选择正确目的地"
			...
			<view class="form-item">
				...
				<view class="form-item-content">
					<input v-model="URDomesticReport.destAddrDetail" :maxlength='100' class="input"
						placeholder="详细地址,例如1号楼1层101室" :readonly="isTrue" />
				</view>
			</view>
			<view class="form-item">
				<view class="form-item-label">
				...
				<view class="form-item-content ">
					<input @click="showEnterPurpose=true" v-model="URDomesticReport.enterPurposeStr" disabled="true"
						class="input" placeholder="请选择" />
				</view>
			</view>
			<view class="form-item">
				...
				<view class="form-item-content textarea">
					<textarea v-model="URDomesticReport.enterPurposeDetail" :maxlength='50'
			...
			<view class="form-item">
				...
				<view class="form-item-content radio">
					<u-radio-group v-model="URDomesticReport.highRisk" :disabled="isTrue">
			...
			<view class="form-item">
				...
				<view class="form-item-content radio">
					<u-radio-group v-model="URDomesticReport.tempNormal" :disabled="isTrue">
		...
		<div class="footer">
			...
			<button @click="submit"
				v-if='URDomesticReport.userReport.permitAudit!=1'>{{!URDomesticReport.userReport.urId?'提交':'修改'}}</button>
		</div>
		<Feedback />
		...
		methods: {
			async load() {
				await Promise.all([
					...
					var user_profession = goods.filter(v => v.dictValue != 9 && v.dictValue != 10 && v
						.dictValue != 11);
			...
			bindPickerChange(val) {
				...
				this.URDomesticReport.companyName = companyName ? companyName.join('/') : ""
			...
			getReportedInfo(id) {
				userDomesticReportedInfo({
					...
					console.log(this.user_transportation)
					console.log(res.transportation)
					this.user_transportation.forEach(item => {
						console.log(item.dictValue == res.transportation, item.dictValue, res
							.transportation)
					...
					this.URDomesticReport = res
					console.log(testImages, this.testImages)
					this.URDriverReport = res
					if (res.userReport.permitAudit == 2) {
						this.promptShow = true
						this.content = res.userReport.rejectReason
					}
					console.log('状态', res.userReport.permitAudit)
			...
			removeDriverReportDests(index) {
				this.URDomesticReport.driverReportDests.splice(index, 1);
			},
			addDriverReportDests() {
				this.URDomesticReport.driverReportDests.push({
					confirm: false, //是否确认
			...
			birthDateConfirm(e) {
				...
				this.URDomesticReport.userReport.enterDate = uni.$u.timeFormat(e.value, 'yyyy-mm-dd 00:00:00')
			},

			businessTypeSelect(info) {
				console.log(info)
				this.showDusinessType = false
				this.URDomesticReport.transportation = info.value[0].dictValue
				this.URDomesticReport.transportationStr = info.value[0].name
			},
			user_enter_purposeSelect(info) {
				console.log(info)
				this.showEnterPurpose = false
				this.URDomesticReport.enterPurpose = info.value[0].dictValue
				this.URDomesticReport.enterPurposeStr = info.value[0].name
			},
			...
			validityPeopleForm() {
				...
				// if (!flight) message = '请输入航班（车次/车牌号)'
			...
			chooseLocation(res) {
				...
				if (res.detail.value && res.detail.value.length > 0) {
					this.URDomesticReport.originAddr = res.detail.value.join('/')
				}
			...
			chooseLocationDestAddr(res) {
				console.log(res.detail.value);
				if (res.detail.value && res.detail.value.length > 0) {
					this.URDomesticReport.destAddr = res.detail.value.join('/')
				}
			...
			submit() {
				...
				console.log(JSON.stringify(this.URDomesticReport))

				if (this.URDomesticReport.userReport.urId && this.URDomesticReport.userReport.permitAudit == 0) {
					this.URDomesticReport.permitAudit = 0
					this.URDomesticReport.userReport.createTime = uni.$u.timeFormat(new Date(), 'yyyy-mm-dd hh:MM:ss')
					updateDomesticReported(this.URDomesticReport, {
						custom: {
							toast: true,
							catch: true
						}
					...
					this.URDomesticReport.userReport.urId = null
					userReported(this.URDomesticReport, {
						custom: {
							toast: true,
							catch: true
						}
			...
			checkPic() {
				...
				for (var i = 0; i < this.testImages.length; i++) {
					if (i == 0) {
						this.URDomesticReport.userReport.hsUrl = this.testImages[i] //	48小时核酸证明
					} else if (i == 1) {
						this.URDomesticReport.userReport.routeUrl = this.testImages[i] //	
					} else if (i == 2) {
						this.URDomesticReport.userReport.healthUrl = this.testImages[i] //	
					} else if (i == 3) {
						this.URDomesticReport.userReport.otherUrl = this.testImages[i] //	
					}
				...
				uni.chooseLocation({
					success: (res) => {
						...
						if (res.errMsg === 'chooseLocation:ok') {
							console.log(res);
							that.URDomesticReport.destAddr = res.address + "(" + res.name + ")"
